...
import { 
  Trophy, Users, Calendar, Trash2, Edit2, Save, Upload, Monitor, 
  Clock, Smartphone, Play, CheckCircle, X, Lock, Loader2, FastForward, 
  Download, Award, Tv, LogOut, AlertTriangle, Shield, PlusCircle, Printer
} from 'lucide-react';
...
export default function App() {
  ...
  const finalRankings = useMemo(() => {
    ...
    ['U50', 'O50'].forEach(cat => {
      ...
      if (!finalMatch || !finalMatch.winnerId) return; // Hide rankings until final is played
  ...
  const generateKO = (qualCount, useWildcards = false) => {
    ...
    // Create dynamically shifting time slots based on court availability
  ...
  // --- REUSABLE MATCH BOX FOR KO TREE (Full Names, Aligned Scores, Custom Colors) ---
  const MonitorMatchBox = ({ matchId, title, fallbackNode }) => {
    const match = matches.find(m => m.id === matchId) || fallbackNode;
    if (!match) return <div className="min-w-[20rem] h-24 border-2 border-dashed border-[var(--contrast-3)] rounded-xl bg-[var(--base)] flex items-center justify-center text-[var(--contrast-2)] text-sm font-bold m-2">Offen</div>;
    
    const t1IsBye = match.team1?.isBye; const t2IsBye = match.team2?.isBye;
    if (t1IsBye && t2IsBye) return null;

    return (
      <div className="min-w-[22rem] border border-[var(--base)] rounded-xl bg-[var(--base-3)] shadow-md overflow-hidden m-2 print:border-[var(--contrast-3)] print:shadow-none break-inside-avoid">
         <div className="bg-[var(--base)] text-xs text-center py-2 font-extrabold text-[var(--contrast-2)] border-b border-[var(--base)] uppercase tracking-wider">{title || formatMatchName(match.groupName)}</div>
         ...
         <div className={`p-3 border-b border-[var(--base)] flex items-center justify-between ${match.winnerId === match.team1?.id ? 'bg-[var(--tcw-green)]/10 font-bold text-[var(--tcw-green-dark)]' : 'text-[var(--contrast)]'}`}>
           <span className="whitespace-nowrap font-medium pr-4">{t1IsBye ? 'Kommt weiter (Freilos)' : (match.team1?.name || 'Offen')}</span>
           {!t1IsBye && !t2IsBye && (
              <div className="flex shrink-0 space-x-2 w-24 justify-end">
                 ...
                 <span className="w-8 text-center text-[var(--tcw-orange)] font-bold">{match.score?.tb ? match.score.tb[0] : ''}</span>
              </div>
           )}
         </div>
         ...
         <div className={`p-3 flex items-center justify-between ${match.winnerId === match.team2?.id ? 'bg-[var(--tcw-green)]/10 font-bold text-[var(--tcw-green-dark)]' : 'text-[var(--contrast)]'}`}>
           <span className="whitespace-nowrap font-medium pr-4">{t2IsBye ? 'Kommt weiter (Freilos)' : (match.team2?.name || 'Offen')}</span>
           {!t1IsBye && !t2IsBye && (
              <div className="flex shrink-0 space-x-2 w-24 justify-end">
                 ...
                 <span className="w-8 text-center text-[var(--tcw-orange)] font-bold">{match.score?.tb ? match.score.tb[1] : ''}</span>
              </div>
           )}
         </div>
      </div>
  ...
  if (appMode === 'login') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 relative bg-[var(--base-2)] text-[var(--contrast)]">
        <div className="absolute inset-0 z-0">
          <img src={BRAND.banner} alt="Background" className="w-full h-full object-cover opacity-[0.03]" />
        </div>
        <div className="relative z-10 w-full max-w-sm flex flex-col items-center">
          <div className="h-24 w-24 bg-transparent flex items-center justify-center p-1 drop-shadow-md mb-6">
             <img src={BRAND.logo} alt="Logo" className="w-full h-full object-contain" />
          </div>
          <div className="bg-[var(--base-3)] p-8 rounded-lg shadow-xl w-full border border-[var(--base)] mb-6 relative overflow-hidden">
            ...
            <h2 className="text-2xl font-extrabold text-center mt-2 mb-2">{BRAND.name} Portal</h2>
            <p className="text-center text-[var(--contrast-2)] text-sm mb-6 font-medium">Veranstalter-Passwort oder Team-PIN</p>
            <form onSubmit={handleLogin} className="space-y-4">
              <input type="password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full p-3 border-2 border-[var(--base)] rounded-md text-center tracking-widest font-bold focus:border-[var(--tcw-green)] focus:ring-0 transition" placeholder="Passwort / PIN" required />
          ...
          <div className="bg-[var(--contrast)] p-8 rounded-lg shadow-xl w-full text-center">
             <Tv className="h-10 w-10 text-[var(--tcw-green-light)] mx-auto mb-4" />
             <h3 className="font-bold text-white text-xl mb-2">TV-Monitor</h3>
             <p className="text-sm text-[var(--contrast-3)] mb-6 font-medium">Live-Turnier-Dashboard für große Bildschirme.</p>
             <button onClick={() => setAppMode('monitor')} className="w-full bg-[var(--contrast-2)] text-white p-3 rounded-md font-bold hover:bg-[var(--contrast-3)] transition flex items-center justify-center">
  ...
  if (appMode === 'player') {
    ...
    return (
      <div className="bg-[var(--base-2)] min-h-screen flex justify-center text-[var(--contrast)]">
        <div className="w-full max-w-[400px] bg-[var(--base-3)] min-h-screen flex flex-col shadow-2xl relative">
          
          <header className="relative bg-[var(--contrast)] text-white pt-10 pb-6 px-5 z-10 overflow-hidden">
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-4">
                <div className="h-14 w-14 bg-transparent flex items-center justify-center drop-shadow-md">
                   <img src={BRAND.logo} alt="Logo" className="w-full h-full object-contain" />
                </div>
                <button onClick={handleLogout} className="text-[var(--contrast-3)] hover:text-white p-2 bg-[var(--contrast-2)]/30 rounded-full">
                  <LogOut size={20} />
                </button>
              </div>
              <h1 className="text-2xl font-bold leading-tight mb-1">{myTeam.name}</h1>
              <div className="flex items-center space-x-2 text-[var(--tcw-green-light)] text-sm font-medium">
          ...
          <main className="flex-1 overflow-y-auto p-5 pb-24">
            {playerTab === 'matches' && (
              <div className="space-y-4">
                <h2 className="text-lg font-extrabold flex items-center"><Calendar className="mr-2 text-[var(--tcw-green)]" size={20}/> Mein Spielplan</h2>
                {scheduledMatches.length === 0 ? (
                  <div className="bg-[var(--base-2)] p-6 rounded-lg text-center text-[var(--contrast-2)] font-medium border border-[var(--base)]">Noch keine Spiele geplant.</div>
                ) : (
            ...
            {playerTab === 'group' && (
              <div className="space-y-4">
                <h2 className="text-lg font-extrabold flex items-center"><Shield className="mr-2 text-[var(--tcw-green)]" size={20}/> {myGroupName}</h2>
            ...
            {playerTab === 'rankings' && (
              <div className="space-y-4">
                <h2 className="text-lg font-extrabold flex items-center"><Award className="mr-2 text-[var(--tcw-yellow)]" size={20}/> Rangliste</h2>
                {!rankingAvailable ? (
                   <div className="bg-[var(--base-2)] p-6 rounded-lg text-center shadow-sm border border-[var(--base)] text-[var(--contrast-2)] font-medium">Die finale Rangliste wird veröffentlicht, sobald das Finale gespielt wurde.</div>
                ) : (
                  <div className="bg-[var(--base-3)] rounded-lg shadow-sm border border-[var(--base)] overflow-hidden">
          ...
          <nav className="absolute bottom-0 w-full bg-[var(--base-3)] border-t border-[var(--base)] flex justify-around p-3 shadow-md">
             <button onClick={() => setPlayerTab('matches')} className={`flex flex-col items-center p-2 rounded-md w-20 transition-colors ${playerTab === 'matches' ? 'text-[var(--tcw-green-dark)] bg-[var(--tcw-green)]/10' : 'text-[var(--contrast-2)]'}`}>
               <Calendar size={20} className="mb-1" /> <span className="text-[10px] font-bold uppercase tracking-wider">Spiele</span>
             </button>
             <button onClick={() => setPlayerTab('group')} className={`flex flex-col items-center p-2 rounded-md w-20 transition-colors ${playerTab === 'group' ? 'text-[var(--tcw-green-dark)] bg-[var(--tcw-green)]/10' : 'text-[var(--contrast-2)]'}`}>
               <Shield size={20} className="mb-1" /> <span className="text-[10px] font-bold uppercase tracking-wider">Gruppe</span>
             </button>
             <button onClick={() => setPlayerTab('rankings')} className={`flex flex-col items-center p-2 rounded-md w-20 transition-colors ${playerTab === 'rankings' ? 'text-[var(--tcw-green-dark)] bg-[var(--tcw-green)]/10' : 'text-[var(--contrast-2)]'}`}>
  ...
  if (appMode === 'monitor') {
    ...
    return (
      <div className="h-screen w-screen overflow-hidden bg-[var(--contrast)] text-[var(--base-3)] font-sans p-6 flex flex-col">
        <header className="relative flex justify-between items-center mb-6 overflow-hidden rounded-xl shadow-2xl border border-[var(--contrast-2)] shrink-0 bg-[var(--contrast)]">
          <div className="relative z-10 flex w-full justify-between items-center p-5">
              <div className="flex items-center space-x-6">
                <div className="h-20 w-20 bg-transparent flex items-center justify-center p-1 drop-shadow-lg">
                   <img src={BRAND.logo} alt="Logo" className="w-full h-full object-contain" />
                </div>
                <div>
                  <h1 className="text-5xl font-extrabold tracking-tight text-white drop-shadow-md">{BRAND.name}</h1>
              ...
              <div className="flex items-center space-x-6">
                <button onClick={handleLogout} className="flex items-center space-x-2 text-[var(--contrast-3)] hover:text-white bg-[var(--contrast-2)]/50 px-4 py-2 rounded-md transition border border-[var(--contrast-2)] text-lg">
                  <Lock size={20} /> <span>Veranstalter</span>
                </button>
                <div className="text-4xl font-bold text-[var(--base-3)] bg-[var(--contrast-2)]/50 px-6 py-3 rounded-md shadow-inner border border-[var(--contrast-2)] min-w-[160px] text-center">
                  {currentTime}
                </div>
              </div>
          </div>
        ...
        <main className="flex-1 overflow-hidden">
              ...
              {[0, 1].map(colIdx => (
                <div key={colIdx} className="flex flex-col space-y-4">
                   {(slide.data || []).slice(colIdx * 5, (colIdx + 1) * 5).map(m => {
                      const matchDate = m.day === 1 ? tournamentStartDate : addDays(tournamentStartDate, 1);
                      return (
                      <div key={m.id} className="bg-[var(--contrast-2)] rounded-lg overflow-hidden shadow-lg border border-[var(--contrast-3)] p-4">
                         <div className="text-[var(--contrast-3)] font-bold mb-3 flex justify-between text-lg border-b border-[var(--contrast-3)] pb-2 uppercase tracking-wider">
                            <span className="text-[var(--base-3)]">{formatDate(matchDate)} • {m.time} • Platz {m.court}</span>
                         ...
                         <div className="flex justify-between items-center text-2xl">
                            <span className={`whitespace-nowrap w-5/12 text-right overflow-hidden text-ellipsis ${m.winnerId === m.team1?.id ? 'text-white font-extrabold' : 'text-[var(--contrast-3)]'}`}>{m.team1?.name || 'Offen'}</span>
                            <span className="w-2/12 text-center bg-[var(--contrast)] py-1 rounded font-bold tracking-widest shadow-inner text-[var(--tcw-green-light)]">
                               {!m.score ? <span className="text-[var(--contrast-2)]">VS</span> : `${m.score.s1[0]}:${m.score.s1[1]}  ${m.score.s2[0]}:${m.score.s2[1]}` + (m.score.tb ? ` [${m.score.tb[0]}:${m.score.tb[1]}]` : '')}
                            </span>
                            <span className={`whitespace-nowrap w-5/12 text-left overflow-hidden text-ellipsis ${m.winnerId === m.team2?.id ? 'text-white font-extrabold' : 'text-[var(--contrast-3)]'}`}>{m.team2?.name || 'Offen'}</span>
                         </div>
                      </div>
                      )
                   })}
          ...
          {slide.type === 'groups' && (
             <div className="grid grid-cols-2 gap-6 h-full content-start">
               {(slide.data || []).map(([groupName, groupTeams]) => (
                 <div key={groupName} className="bg-[var(--contrast-2)] rounded-lg border border-[var(--contrast-3)] overflow-hidden shadow-xl">
                   <div className="bg-[var(--contrast)] p-3 text-2xl font-bold text-center text-[var(--base-3)] tracking-widest">{groupName}</div>
                   <table className="w-full text-xl">
                     <thead className="bg-[var(--contrast)]/50 text-[var(--contrast-3)]"><tr><th className="p-3 text-left pl-6">Team</th><th className="p-3 text-center">S-N</th><th className="p-3 text-center">Sätze</th></tr></thead>
                     <tbody>
                       {(standings[slide.cat]?.[groupName] || groupTeams).map((t, idx) => (
                         <tr key={t.id} className="border-b border-[var(--contrast-3)] last:border-0">
                           <td className="p-4 pl-6 font-medium text-[var(--base-3)] truncate flex items-center"><span className="text-[var(--contrast-3)] w-8 font-bold">{idx+1}</span> {t.name}</td>
                           <td className="p-4 text-center font-bold text-white bg-[var(--contrast)]/20">{t.won !== undefined ? `${t.won}-${t.lost}` : '0-0'}</td>
                           <td className="p-4 text-center text-[var(--contrast-3)] bg-[var(--contrast)]/20">{t.setsWon !== undefined ? `${t.setsWon}:${t.setsLost}` : '0:0'}</td>
                         </tr>
                       ))}
                     </tbody>
                   </table>
          ...
          {slide.type === 'bracket_main' && brackets[slide.cat] && (
             <div className="h-full flex items-center justify-center pb-8 overflow-x-auto">
                 <div className="flex items-center space-x-12 min-w-max relative px-12">
                     <div className="flex flex-col justify-around space-y-4 h-[70vh]">
                        {brackets[slide.cat].qf.map((qfRef, i) => <div key={i} className="relative"><MonitorMatchBox matchId={qfRef.id} title={`Viertelfinale ${i+1}`} fallbackNode={qfRef} /><div className="absolute top-1/2 -right-6 w-6 border-b-2 border-[var(--contrast-3)]"></div></div>)}
                     </div>
                     <div className="flex flex-col justify-around h-[70vh]">
                        {brackets[slide.cat].sf.map((sfRef, i) => <div key={i} className="relative"><div className="absolute top-[-100px] -left-6 bottom-[50%] border-l-2 border-[var(--contrast-3)] rounded-tl-lg"></div><div className="absolute bottom-[-100px] -left-6 top-[50%] border-l-2 border-[var(--contrast-3)] rounded-bl-lg"></div><div className="absolute top-1/2 -left-6 w-6 border-b-2 border-[var(--contrast-3)]"></div><MonitorMatchBox matchId={sfRef.id} title={sfRef.title} fallbackNode={sfRef} /><div className="absolute top-1/2 -right-6 w-6 border-b-2 border-[var(--contrast-3)]"></div></div>)}
                     </div>
                     <div className="flex flex-col justify-center h-[70vh]">
                        <div className="relative"><div className="absolute top-[-150px] -left-6 bottom-[50%] border-l-2 border-[var(--contrast-3)] rounded-tl-lg"></div><div className="absolute bottom-[-150px] -left-6 top-[50%] border-l-2 border-[var(--contrast-3)] rounded-bl-lg"></div><div className="absolute top-1/2 -left-6 w-6 border-b-2 border-[var(--contrast-3)]"></div><MonitorMatchBox matchId={brackets[slide.cat].finals[0].id} title="FINALE" fallbackNode={brackets[slide.cat].finals[0]} /></div>
                     </div>
                 </div>
             </div>
          )}
             ...
             <div className="h-full flex justify-center items-center pb-12 overflow-x-auto">
                 <div className="flex items-start space-x-12 min-w-max">
                     <div className="flex flex-col justify-around space-y-6">
                        {brackets[slide.cat].pSf.map((pSfRef, i) => <MonitorMatchBox key={i} matchId={pSfRef.id} title={pSfRef.title} fallbackNode={pSfRef} />)}
                        <MonitorMatchBox matchId={brackets[slide.cat].finals[1].id} title="Spiel um Platz 3" fallbackNode={brackets[slide.cat].finals[1]} />
                     </div>
                     <div className="flex flex-col justify-around space-y-6">
                        <MonitorMatchBox matchId={brackets[slide.cat].finals[2].id} title="Spiel um Platz 5" fallbackNode={brackets[slide.cat].finals[2]} />
                        <MonitorMatchBox matchId={brackets[slide.cat].finals[3].id} title="Spiel um Platz 7" fallbackNode={brackets[slide.cat].finals[3]} />
                     </div>
                 </div>
             </div>
          )}
             ...
             <div className="grid grid-cols-2 gap-8 h-full content-start">
                <div className="flex flex-col space-y-4">
                   {(slide.data || []).slice(0, 6).map(item => (
                      <div key={item.team.id} className={`rounded-lg border flex items-center p-5 shadow-lg ${item.rank === 1 ? 'bg-[var(--tcw-yellow)]/20 border-[var(--tcw-yellow)]/50' : item.rank === 2 ? 'bg-[var(--contrast-3)]/20 border-[var(--contrast-3)]' : item.rank === 3 ? 'bg-[var(--global-color-12)]/20 border-[var(--global-color-12)]/50' : 'bg-[var(--contrast-2)] border-[var(--contrast-3)]'}`}>
                         <div className={`text-4xl font-black w-20 text-center ${item.rank === 1 ? 'text-[var(--tcw-yellow)]' : item.rank === 2 ? 'text-[var(--contrast-3)]' : item.rank === 3 ? 'text-[var(--global-color-12)]' : 'text-[var(--contrast-3)]'}`}>
                ...
                <div className="flex flex-col space-y-4">
                   {(slide.data || []).slice(6, 12).map(item => (
                      <div key={item.team.id} className="bg-[var(--contrast-2)] rounded-lg border border-[var(--contrast-3)] flex items-center p-5 shadow-lg">
          ...
          {slide.type === 'idle' && (
             <div className="flex items-center justify-center h-full flex-col text-[var(--contrast-2)] space-y-6">
                <Trophy size={120} className="text-[var(--contrast-3)]" />
  ...
  // ORGANIZER DASHBOARD
  return (
    <div className={`min-h-screen bg-[var(--base-2)] text-[var(--contrast)] pb-12 relative ${printView === 'sheets' ? 'bg-[var(--base-3)]' : ''}`}>
      <style dangerouslySetInnerHTML={{__html: `
        ...
        h1, h2, h3, h4, h5, h6, .font-extrabold, .font-black { font-family: 'Montserrat', sans-serif; }
      ...
      <header className={`relative bg-[var(--base-3)] text-[var(--contrast)] shadow-md overflow-hidden border-b border-[var(--base)] ${printView === 'sheets' ? 'hidden' : 'print:hidden'}`}>
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-extrabold hidden md:block tracking-tight text-[var(--tcw-green)]">{BRAND.name}</h1>
              <div className="text-xs font-bold text-[var(--contrast-2)] uppercase tracking-widest hidden md:block">Turnier-Veranstalter</div>
            </div>
          </div>
          <div className="flex space-x-2">
            <button onClick={handleSimulateTournament} disabled={simState !== 'idle'} className={`flex items-center space-x-2 px-3 py-2 rounded-md font-bold transition text-sm text-white ${simState !== 'idle' ? 'bg-[var(--contrast-3)] cursor-not-allowed' : 'bg-[var(--tcw-orange)] hover:bg-[var(--global-color-13)]'}`}>
              {simState !== 'idle' ? <Loader2 size={18} className="animate-spin" /> : <FastForward size={18} />} <span className="hidden sm:inline">Simulation</span>
            </button>
            <button onClick={() => window.print()} className="flex items-center space-x-2 bg-[var(--base)] hover:bg-[var(--contrast-3)] text-[var(--contrast)] px-3 py-2 rounded-md transition text-sm font-bold border border-[var(--contrast-3)]/30">
              <Printer size={18} /> <span className="hidden sm:inline">Drucken</span>
            </button>
            <button onClick={() => setAppMode('monitor')} className="flex items-center space-x-2 bg-[var(--contrast)] hover:bg-[var(--contrast-2)] text-white px-3 py-2 rounded-md transition text-sm font-bold">
      ...
      <main className={`max-w-7xl mx-auto px-4 py-8 ${printView === 'sheets' ? 'hidden' : 'print:p-0 print:max-w-none'}`}>
        <div className="flex space-x-1 bg-[var(--base-3)] border border-[var(--base)] p-1 rounded-lg mb-8 print:hidden shadow-sm">
          {[{ id: 'registration', icon: Users, label: 'Anmeldung' }, { id: 'groups', icon: Shield, label: 'Gruppen' }, { id: 'schedule', icon: Calendar, label: 'Spielplan' }, { id: 'bracket', icon: Trophy, label: 'K.O.-Baum' }, { id: 'rankings', icon: Award, label: 'Rangliste' }].map((t) => (
            <button key={t.id} onClick={() => setActiveTab(t.id)} className={`flex-1 flex items-center justify-center space-x-2 py-3 px-4 rounded-md font-bold transition-all ${activeTab === t.id ? 'bg-[var(--tcw-green)]/10 text-[var(--tcw-green-dark)]' : 'text-[var(--contrast-2)] hover:bg-[var(--base)]'}`}>
              <t.icon size={18} /> <span>{t.label}</span>
            </button>
          ))}
        </div>

        <div className="bg-[var(--base-3)] rounded-lg shadow-sm border border-[var(--base)] p-6 print:border-none print:shadow-none print:p-0">
          ...
          {activeTab === 'registration' && (
            <div className="space-y-8 print:hidden">
              <div className="flex justify-between items-center bg-[var(--base-2)] p-6 rounded-lg border border-[var(--base)]">
                <div>
                  <h2 className="text-xl font-extrabold">Turnier-Einstellungen</h2>
                  <p className="text-[var(--contrast-2)] text-sm mt-1 font-medium">Teams registrieren oder Testdaten laden.</p>
                </div>
                <button onClick={loadMockData} className="flex items-center space-x-2 bg-[var(--base-3)] text-[var(--contrast)] border border-[var(--base)] px-5 py-2.5 rounded-md shadow-sm hover:bg-[var(--base)] font-bold transition">
                  <Play size={18} className="text-[var(--tcw-green)]" /> <span>Test-Teams laden</span>
                </button>
              </div>

              <div className="bg-[var(--base-2)] p-4 rounded-lg border border-[var(--base)]">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 pb-4 border-b border-[var(--base)]">
                   <div className="flex items-center space-x-3">
                     <Calendar className="text-[var(--contrast-2)]" />
                     <div className="font-bold">Turnier-Startdatum:</div>
                     <input type="date" value={tournamentStartDate} onChange={(e) => setTournamentStartDate(e.target.value)} className="p-2 border border-[var(--contrast-3)] rounded-md font-bold text-[var(--tcw-green-dark)] focus:border-[var(--tcw-green)] outline-none" />
                   </div>
                   <div className="flex items-center space-x-3">
                     <Clock className="text-[var(--contrast-2)]" />
                     <div className="font-bold">Dauer:</div>
                     <select value={tournamentDays} onChange={(e) => setTournamentDays(Number(e.target.value))} className="p-2 border border-[var(--contrast-3)] rounded-md font-bold focus:border-[var(--tcw-green)] outline-none">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center space-x-3">
                     <Clock className="text-[var(--contrast-2)]" />
                     <div className="font-bold">{tournamentDays === 1 ? 'Turnier-Startzeit:' : 'Startzeit Tag 1:'}</div>
                     <input type="time" value={day1Start} onChange={(e) => setDay1Start(e.target.value)} className="p-2 border border-[var(--contrast-3)] rounded-md font-bold text-[var(--tcw-green-dark)] focus:border-[var(--tcw-green)] outline-none" />
                  </div>
                  {tournamentDays === 2 && (
                    <div className="flex items-center space-x-3">
                       <Clock className="text-[var(--contrast-2)]" />
                       <div className="font-bold">Startzeit Tag 2 (K.O.):</div>
                       <input type="time" value={day2Start} onChange={(e) => setDay2Start(e.target.value)} className="p-2 border border-[var(--contrast-3)] rounded-md font-bold text-[var(--tcw-green-dark)] focus:border-[var(--tcw-green)] outline-none" />
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="col-span-1 bg-[var(--base-2)] p-6 rounded-lg border border-[var(--base)] h-fit">
                  <h3 className="font-extrabold text-lg mb-4 flex items-center">
                  ...
                  <form onSubmit={handleRegister} className="space-y-4">
                    <div className="p-3 bg-[var(--base-3)] border border-[var(--base)] rounded-md space-y-3">
                      ...
                      <input value={regForm.p1Name} onChange={e=>setRegForm({...regForm, p1Name: e.target.value})} className="w-full p-2 border border-[var(--base)] rounded-md text-sm font-medium focus:border-[var(--tcw-green)] outline-none" placeholder="Vollständiger Name" required />
                      <input value={regForm.p1Club} onChange={e=>setRegForm({...regForm, p1Club: e.target.value})} className="w-full p-2 border border-[var(--base)] rounded-md text-sm font-medium focus:border-[var(--tcw-green)] outline-none" placeholder="Verein (Optional)" />
                    </div>
                    <div className="p-3 bg-[var(--base-3)] border border-[var(--base)] rounded-md space-y-3">
                      <div className="text-xs font-bold text-[var(--contrast-3)] uppercase tracking-wider">Spieler 2</div>
                      <input value={regForm.p2Name} onChange={e=>setRegForm({...regForm, p2Name: e.target.value})} className="w-full p-2 border border-[var(--base)] rounded-md text-sm font-medium focus:border-[var(--tcw-green)] outline-none" placeholder="Vollständiger Name" required />
                      <input value={regForm.p2Club} onChange={e=>setRegForm({...regForm, p2Club: e.target.value})} className="w-full p-2 border border-[var(--base)] rounded-md text-sm font-medium focus:border-[var(--tcw-green)] outline-none" placeholder="Verein (Optional)" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div><label className="block text-sm font-bold mb-1">Stärke</label>
                        <select value={regForm.level} onChange={e=>setRegForm({...regForm, level: e.target.value})} className="w-full p-2 border border-[var(--base)] rounded-md font-medium outline-none">
                          <option value="3">3 - Gut</option><option value="2">2 - Mittel</option><option value="1">1 - Anfänger</option>
                        </select></div>
                      <div><label className="block text-sm font-bold mb-1">Kat.</label>
                        <select value={regForm.category} onChange={e=>setRegForm({...regForm, category: e.target.value})} className="w-full p-2 border border-[var(--base)] rounded-md font-medium outline-none">
                          <option value="U50">U50</option><option value="O50">O50</option>
                        </select></div>
                    </div>
                    <div className="flex space-x-2 pt-2">
                      <button type="submit" className={`flex-1 text-white py-2.5 rounded-md font-bold transition ${editingTeam ? 'bg-[var(--tcw-orange)] hover:bg-[var(--global-color-13)]' : 'bg-[var(--tcw-green)] hover:bg-[var(--tcw-green-dark)]'}`}>
                        {editingTeam ? 'Aktualisieren' : 'Registrieren'}
                      </button>
                      {editingTeam && <button type="button" onClick={() => {setEditingTeam(null); setRegForm({ p1Name: '', p1Club: '', p2Name: '', p2Club: '', level: '2', category: 'U50' });}} className="px-4 bg-[var(--base)] text-[var(--contrast)] rounded-md font-bold hover:bg-[var(--contrast-3)] transition">Abbruch</button>}
                    </div>
                  </form>
                </div>
                
                <div className="col-span-2">
                  <h3 className="font-extrabold text-lg mb-4">Registrierte Teams ({teams.length})</h3>
                  <div className="overflow-auto max-h-[600px] border border-[var(--base)] rounded-lg shadow-inner bg-[var(--base-2)]">
                    <table className="w-full text-left text-sm table-fixed">
                      <thead className="bg-[var(--base)] sticky top-0 z-10 text-[var(--contrast-2)]">
                        <tr>
                          <th className="p-3 border-b border-[var(--contrast-3)] w-1/3">Team</th>
                          <th className="p-3 border-b border-[var(--contrast-3)] w-1/4">Vereine</th>
                          <th className="p-3 border-b border-[var(--contrast-3)] w-20">Kat</th>
                          <th className="p-3 border-b border-[var(--contrast-3)] w-24 text-center">PIN</th>
                          <th className="p-3 border-b border-[var(--contrast-3)] w-24"></th>
                        </tr>
                      </thead>
                      <tbody>
                        {teams.length === 0 ? <tr><td colSpan="5" className="p-8 text-center text-[var(--contrast-2)] font-medium">Noch keine Teams registriert.</td></tr> : 
                         teams.map((t) => (
                          <tr key={t.id} className="border-b border-[var(--base)] last:border-0 hover:bg-[var(--base-3)] transition bg-white">
                            <td className="p-3 font-bold truncate">{t.name}</td>
                            <td className="p-3 text-xs font-medium text-[var(--contrast-2)] truncate">{t.clubs.join(' / ') || '-'}</td>
                            <td className="p-3"><span className="px-2 py-1 rounded text-xs font-bold bg-[var(--base)] text-[var(--contrast-2)]">{t.category}</span></td>
                            <td className="p-3 text-center font-mono font-bold text-[var(--tcw-green-dark)] bg-[var(--tcw-green)]/10">{t.pin}</td>
                            <td className="p-3 text-right space-x-2">
                               <button onClick={() => handleEdit(t)} className="text-[var(--contrast-3)] hover:text-[var(--global-color-12)] transition"><Edit2 size={16}/></button>
                               <button onClick={() => confirmDelete === t.id ? handleDelete(t.id) : setConfirmDelete(t.id)} className={`transition ${confirmDelete === t.id ? 'text-white bg-[var(--tcw-orange)] px-2 rounded font-bold' : 'text-[var(--contrast-3)] hover:text-[var(--tcw-orange)]'}`}>
          ...
          {activeTab === 'groups' && (
            <div className="space-y-8">
              <div className="flex justify-between items-center print:hidden">
                <p className="text-[var(--contrast-2)] font-medium">Ausbalancierte Gruppen.</p>
                <div className="space-x-3">
                   <button onClick={generateGroups} disabled={teams.length < 4} className="bg-[var(--base)] text-[var(--contrast)] hover:bg-[var(--contrast-3)] px-4 py-2 rounded-md font-bold disabled:opacity-50 transition border border-[var(--contrast-3)]/30">1. Generieren</button>
                   <button onClick={() => setSchedulePrompt(true)} disabled={Object.keys(groups.U50).length === 0 && Object.keys(groups.O50).length === 0} className="bg-[var(--tcw-green)] text-white hover:bg-[var(--tcw-green-dark)] px-4 py-2 rounded-md font-bold shadow-sm disabled:opacity-50 transition">2. Spielplan</button>
                </div>
              </div>
              ...
              {['U50', 'O50'].map(cat => (
                 groups[cat] && Object.keys(groups[cat]).length > 0 && (
                   <div key={cat} className="mb-10 page-break-after">
                     <h2 className="text-2xl font-extrabold mb-6 border-b-2 border-[var(--base)] pb-2 flex items-center">
                       {CATEGORIES[cat]}
                     </h2>
                     <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {Object.keys(groups[cat]).sort().map((groupName) => (
                          <div key={groupName} className="border border-[var(--base)] rounded-lg overflow-hidden shadow-sm">
                            <div className="bg-[var(--base-2)] p-3 font-extrabold text-[var(--tcw-green-dark)] border-b border-[var(--base)]">{groupName}</div>
                            <table className="w-full text-sm text-left table-fixed">
                              <thead className="bg-[var(--base-3)] text-[var(--contrast-3)] uppercase text-xs">
                                <tr>
                              ...
                              <tbody>
                                {(standings[cat][groupName] || groups[cat][groupName]).map((t, index) => (
                                  <tr key={t.id} className="border-b border-[var(--base)] last:border-0 hover:bg-[var(--base-2)] bg-[var(--base-3)] transition">
                                    ...
                                    <td className="px-4 py-3 text-center font-bold">{t.won !== undefined ? `${t.won}-${t.lost}` : '0-0'}</td>
          ...
          {activeTab === 'schedule' && (
            <div>
               <div className="flex justify-between items-center mb-6 print:hidden">
                 <h2 className="text-xl font-extrabold">Spielplan</h2>
                 <div className="flex space-x-3">
                   {matches.some(m => m.day === 1 && m.time === 'Flexibel') && <button onClick={() => { setPrintView('sheets'); setTimeout(() => { window.print(); setPrintView('normal'); }, 150); }} className="bg-[var(--contrast)] text-white px-4 py-2 rounded-md font-bold hover:bg-[var(--contrast-2)] transition shadow-sm flex items-center"><Printer size={18} className="mr-2"/> Drucken</button>}
                   <button onClick={handleKoGeneration} disabled={matches.filter(m => m.stage === 'Group' && !m.winnerId).length > 0 || matches.length === 0} className={`px-5 py-2 rounded-md font-bold transition flex items-center ${matches.filter(m => m.stage === 'Group' && !m.winnerId).length === 0 && matches.length > 0 ? 'bg-[var(--tcw-green)] text-white hover:bg-[var(--tcw-green-dark)]' : 'bg-[var(--base)] text-[var(--contrast-3)] cursor-not-allowed'}`}>
                 ...
                 {[1, 2].map(day => {
                    ...
                    return (
                      <div key={day} className="mb-8">
                        <h3 className="text-lg font-bold bg-[var(--tcw-green-dark)] text-white p-4 rounded-t-lg flex justify-between items-center shadow-sm">
                          <span>{tournamentDays === 1 ? 'Alle Spiele (1-Tages-Turnier)' : `Tag ${day} (${formatDate(dateStr)}) ${day===1 ? '- Gruppenphase' : '- K.O. Runde'}`}</span>
                        </h3>
                        <div className="border-x border-b border-[var(--base)] rounded-b-lg overflow-hidden bg-[var(--base-3)] shadow-sm">
                          <table className="w-full text-sm table-fixed">
                            <thead className="bg-[var(--base-2)] text-[var(--contrast-2)] uppercase text-xs">
                              <tr>
                                <th className="p-3 text-left w-20 border-b border-[var(--base)]">Zeit</th>
                                <th className="p-3 text-left w-20 border-b border-[var(--base)]">Platz</th>
                                <th className="p-3 text-left w-32 border-b border-[var(--base)]">Phase</th>
                            ...
                            <tbody>
                              {[...dayMatches, ...unscheduled].map((m) => {
                                ...
                                <tr key={m.id} className={`border-b border-[var(--base)] last:border-0 transition ${isMissingTeams || isBye || m.time === null ? '' : 'cursor-pointer hover:bg-[var(--base-2)]'}`} onClick={() => !isMissingTeams && !isBye && m.time !== null && setScoreModal(m)}>
                                  <td className={`p-3 font-bold ${m.time === null ? 'text-[var(--tcw-orange)]' : (m.time === 'Flexibel' ? 'text-[var(--contrast-3)] text-xs tracking-widest' : 'text-[var(--contrast)]')}`}>{m.time || 'Nicht angesetzt'}</td>
                                  <td className="p-3">{m.court ? <span className="bg-[var(--base)] text-[var(--contrast-2)] px-2 py-1 rounded-md text-xs font-bold">Platz {m.court}</span> : '-'}</td>
                                  <td className="p-3 text-xs truncate"><span className="block font-bold text-[var(--tcw-green-dark)]">{CATEGORIES[m.category]?.substring(0,3)} {m.category}</span><span className="text-[var(--contrast-2)] font-medium">{formatMatchName(m.groupName)}</span></td>
                                  <td className={`p-3 text-right truncate ${isT1Winner ? 'font-bold text-[var(--tcw-green)]' : 'font-medium'}`}>
                                    {m.team1?.isBye ? <span className="text-[var(--contrast-3)] italic">Freilos</span> : (m.team1?.name || 'Offen')}
                                  </td>
                                  <td className="p-3 text-center bg-[var(--base-2)] border-x border-[var(--base)] font-bold">
                                  ...
                                  <td className={`p-3 text-left truncate ${isT2Winner ? 'font-bold text-[var(--tcw-green)]' : 'font-medium'}`}>
             ...
             <div className="overflow-x-auto pb-12">
               {['U50', 'O50'].map(cat => {
                 ...
                 const BracketMatchBox = ({ match, title }) => {
                   if (!match) return <div className="min-w-[20rem] h-24 border-2 border-dashed border-[var(--contrast-3)] rounded-md bg-[var(--base)] flex items-center justify-center text-[var(--contrast-2)] text-sm font-bold m-2">Offen</div>;
                   const t1IsBye = match.team1?.isBye; const t2IsBye = match.team2?.isBye;
                   if (t1IsBye && t2IsBye) return null;
                   return (
                     <div className="min-w-[20rem] border border-[var(--base)] rounded-md bg-[var(--base-3)] shadow-sm overflow-hidden m-2 print:border-[var(--contrast-3)] print:shadow-none break-inside-avoid">
                        <div className="bg-[var(--base)] text-xs text-center py-1 font-extrabold text-[var(--contrast-2)] border-b border-[var(--base)] uppercase tracking-wider">{title || formatMatchName(match.groupName)}</div>
                        
                        <div className={`p-2 border-b border-[var(--base)] flex items-center justify-between ${match.winnerId === match.team1?.id ? 'bg-[var(--tcw-green)]/10 font-bold text-[var(--tcw-green-dark)]' : 'text-[var(--contrast)]'}`}>
                          <span className="whitespace-nowrap font-medium pr-4">{t1IsBye ? 'Kommt weiter (Freilos)' : (match.team1?.name || 'Offen')}</span>
                          {!t1IsBye && !t2IsBye && (
                             <div className="flex shrink-0 space-x-1 w-24 justify-end">
                                <span className="w-6 text-center font-bold">{match.score?.s1[0]}</span>
                                <span className="w-6 text-center font-bold">{match.score?.s2[0]}</span>
                                <span className="w-8 text-center text-[var(--tcw-orange)] font-bold">{match.score?.tb ? match.score.tb[0] : ''}</span>
                             </div>
                        ...
                        <div className={`p-2 flex items-center justify-between ${match.winnerId === match.team2?.id ? 'bg-[var(--tcw-green)]/10 font-bold text-[var(--tcw-green-dark)]' : 'text-[var(--contrast)]'}`}>
                          <span className="whitespace-nowrap font-medium pr-4">{t2IsBye ? 'Kommt weiter (Freilos)' : (match.team2?.name || 'Offen')}</span>
                          {!t1IsBye && !t2IsBye && (
                             <div className="flex shrink-0 space-x-1 w-24 justify-end">
                                <span className="w-6 text-center font-bold">{match.score?.s1[1]}</span>
                 ...
                 return (
                   <div key={cat} className="mb-16 page-break-after">
                      <h2 className="text-2xl font-extrabold mb-8 border-b-2 border-[var(--base)] pb-2 flex items-center">
                         <Trophy className="text-[var(--tcw-green)] mr-3"/> Hauptrunde: {CATEGORIES[cat]}
                      </h2>
                      ...
                      <h3 className="text-xl font-bold mt-12 mb-4 border-b border-[var(--base)] pb-2">Platzierungsspiele (Plätze 3-8)</h3>
                      <div className="flex items-start space-x-12 min-w-max">
                        <div className="flex flex-col justify-around space-y-4">
                      ...
                      <div className="mt-4"><BracketMatchBox match={getMatchData(brackets[cat].finals[1].id)} title="Spiel um Platz 3" /></div>
                   </div>
                 )
               })}
             </div>
               ...
               {['U50', 'O50'].map(cat => {
                 ...
                 if (!finalMatch || !finalMatch.winnerId) return (
                    <div key={cat} className="mb-8">
                       <h2 className="text-2xl font-extrabold mb-6 border-b-2 border-[var(--base)] pb-2 flex items-center">
                         <Award className="mr-3 text-[var(--contrast-3)]" /> Abschlussplatzierungen: {CATEGORIES[cat]}
                       </h2>
                       <div className="bg-[var(--base-2)] p-6 rounded-lg text-center font-medium border border-[var(--base)] text-[var(--contrast-2)]">Die finale Rangliste für diese Kategorie wird veröffentlicht, sobald das Finale abgeschlossen ist.</div>
                    </div>
                 );
                 ...
                 return (
                   <div key={cat} className="page-break-after">
                      ...
                      <div className="border border-[var(--base)] rounded-lg overflow-hidden shadow-sm">
                        <table className="w-full text-left text-sm table-fixed">
                           <thead className="bg-[var(--base-2)] text-[var(--contrast-2)] uppercase text-xs">
                             <tr>
                           ...
                           <tbody>
                             {finalRankings[cat].map((item, idx) => (
                               <tr key={item.team.id} className={`border-b border-[var(--base)] last:border-0 hover:bg-[var(--base-2)] transition bg-[var(--base-3)]`}>
                                 ...
                                 <td className="p-4 text-center">
                                   {idx < 8 ? <span className="bg-[var(--tcw-green)]/10 text-[var(--tcw-green-dark)] px-2 py-1 rounded text-xs font-bold">K.O.-Phase</span> : <span className="bg-[var(--base)] text-[var(--contrast-2)] px-2 py-1 rounded text-xs font-bold">Gruppen</span>}
                                 </td>
                               </tr>
                             ))}
                           </tbody>
      ...
      {scoreModal && (
        <div className="fixed inset-0 bg-[var(--contrast)]/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--base-3)] rounded-lg shadow-2xl w-full max-w-md overflow-hidden">
            <div className="bg-[var(--tcw-green)] p-4 flex justify-between items-center text-white">
               <h3 className="font-bold">Ergebnis eintragen</h3>
            ...
            <form onSubmit={(e) => {
              ...
              <div className="flex justify-between items-center mb-4">
                <div className="w-1/2 font-bold truncate pr-2">{scoreModal.team1?.name}</div>
                <input name="s1_t1" type="number" min="0" max="7" defaultValue={scoreModal.score?.s1[0]} className="w-12 p-2 border border-[var(--contrast-3)] rounded text-center font-bold outline-none focus:border-[var(--tcw-green)]" required />
                <input name="s2_t1" type="number" min="0" max="7" defaultValue={scoreModal.score?.s2[0]} className="w-12 p-2 border border-[var(--contrast-3)] rounded text-center font-bold outline-none focus:border-[var(--tcw-green)]" required />
                <input name="tb_t1" type="number" min="0" max="30" defaultValue={scoreModal.score?.tb?.[0]} className="w-12 p-2 border border-[var(--tcw-orange)]/50 rounded text-center font-bold text-[var(--tcw-orange)] outline-none focus:border-[var(--tcw-orange)]" />
              </div>
              <div className="flex justify-between items-center mb-8">
                <div className="w-1/2 font-bold truncate pr-2">{scoreModal.team2?.name}</div>
                <input name="s1_t2" type="number" min="0" max="7" defaultValue={scoreModal.score?.s1[1]} className="w-12 p-2 border border-[var(--contrast-3)] rounded text-center font-bold outline-none focus:border-[var(--tcw-green)]" required />
                <input name="s2_t2" type="number" min="0" max="7" defaultValue={scoreModal.score?.s2[1]} className="w-12 p-2 border border-[var(--contrast-3)] rounded text-center font-bold outline-none focus:border-[var(--tcw-green)]" required />
                <input name="tb_t2" type="number" min="0" max="30" defaultValue={scoreModal.score?.tb?.[1]} className="w-12 p-2 border border-[var(--tcw-orange)]/50 rounded text-center font-bold text-[var(--tcw-orange)] outline-none focus:border-[var(--tcw-orange)]" />
              </div>
              <button type="submit" className="w-full bg-[var(--tcw-green)] text-white py-3 rounded-md font-bold hover:bg-[var(--tcw-green-dark)] transition">Speichern</button>
            </form>
          </div>
        </div>
      )}
      ...
      {schedulePrompt && (
        <div className="fixed inset-0 bg-[var(--contrast)]/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--base-3)] rounded-lg shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="bg-[var(--tcw-green)] p-4 flex justify-between items-center text-white"><h3 className="font-bold">Spielplan-Methode</h3><button onClick={() => setSchedulePrompt(false)}><X size={20}/></button></div>
            <div className="p-6">
              <button onClick={() => generateSchedule('traditional')} className="w-full bg-[var(--base-2)] border border-[var(--base)] p-4 rounded-md font-bold hover:bg-[var(--base)] mb-3 text-left">1. Klassische Zeitfenster</button>
              <button onClick={() => generateSchedule('courtPerGroup')} className="w-full bg-[var(--tcw-green)]/10 border border-[var(--tcw-green)]/30 text-[var(--tcw-green-dark)] p-4 rounded-md font-bold hover:bg-[var(--tcw-green)]/20 text-left">2. Flexible Zeiten (Plätze an Gruppen zuweisen)</button>
            </div>
          </div>
        </div>
      )}
      ...
      {koPrompt && (
        <div className="fixed inset-0 bg-[var(--contrast)]/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--base-3)] rounded-lg shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="bg-[var(--tcw-orange)] p-4 flex justify-between items-center text-white"><h3 className="font-bold flex items-center"><AlertTriangle size={20} className="mr-2"/> Auffüllen für 8-Team-Baum</h3><button onClick={() => setKoPrompt(false)}><X size={20}/></button></div>
            <div className="p-6 flex flex-col space-y-3">
               <button onClick={() => generateKO(2, false)} className="bg-[var(--base-2)] border border-[var(--base)] py-3 px-4 rounded-md font-bold hover:bg-[var(--base)] text-left flex justify-between"><span>1. Plätze leer lassen</span><span className="text-xs bg-[var(--contrast-3)] px-2 py-1 rounded text-white">Freilose</span></button>
               <button onClick={() => generateKO(2, true)} className="bg-[var(--tcw-green)]/10 border border-[var(--tcw-green)]/30 text-[var(--tcw-green-dark)] py-3 px-4 rounded-md font-bold hover:bg-[var(--tcw-green)]/20 text-left flex justify-between"><span>2. Beste verbleibende Teams</span><span className="text-xs bg-[var(--tcw-green)] text-white px-2 py-1 rounded">Wildcards</span></button>
            </div>
          </div>
        </div>
      )}
