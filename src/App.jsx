...
export default function App() {
  ...
  const handleSaveScore = (matchId, s1, s2, tb) => {
    setMatches(prev => prev.map(m => {
      ...
      // Satz 1 auswerten
      if (s1[0] > s1[1]) t1Sets++; 
      else if (s1[1] > s1[0]) t2Sets++;
      
      // Satz 2 auswerten
      if (s2[0] > s2[1]) t1Sets++; 
      else if (s2[1] > s2[0]) t2Sets++;
      
      // Satz 3 / Match-Tiebreak nur auswerten, wenn Satzgleichstand (1:1) herrscht
      if (t1Sets === 1 && t2Sets === 1 && tb) {
        if (tb[0] > tb[1]) t1Sets++; 
        else if (tb[1] > tb[0]) t2Sets++;
      }
      
      // Eindeutige winnerId nur setzen, wenn ein echtes Ergebnis feststeht
      let winnerId = null;
      if (t1Sets > t2Sets) {
        winnerId = m.team1?.id || null;
      } else if (t2Sets > t1Sets) {
        winnerId = m.team2?.id || null;
      }
